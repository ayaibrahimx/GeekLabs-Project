...
function FilterBtn() {
  ...
  return (
    <>
      <div
        className={`absolute left-24 top-0 h-auto w-96 flex-col rounded-md bg-sidebg p-3 ${closed ? 'hidden' : ''}`}
      >
        <IoClose
          className="absolute right-2 top-1 h-5 w-5 cursor-pointer text-white"
          onClick={() => {
            setIsClosed(!closed);
